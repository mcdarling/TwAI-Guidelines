...
checks them and writes:
  ...
  entries/notes.tex           the Useful Notes section, from dictionary/notes.yaml
  entries/acronyms.tex        the Acronyms section, from dictionary/acronyms.yaml
                              and every acronym used as a term's alias
...
ACRONYM = re.compile(r'^[A-Z][A-Za-z]*$')


def load_extras(sources, domains, seen):
    """Load the Useful Notes and the acronym list, and check their links."""
    notes = yaml.safe_load((ROOT / 'dictionary/notes.yaml').read_text())['notes']
    extra = yaml.safe_load((ROOT / 'dictionary/acronyms.yaml').read_text())['acronyms']
    errors = []
    for n in notes:
        if n['source'] not in sources:
            errors.append(f'note {n["key"]}: unknown source {n["source"]!r}')
        for r in n.get('related', []):
            if r not in seen:
                errors.append(f'note {n["key"]}: related key {r!r} does not exist')
    # Every alias that is an acronym, such as LLM or GenAI, is listed too.
    acronyms = {}
    for dom in domains:
        for t in dom['terms']:
            for alias in (t.get('alias') or '').split(', '):
                if ACRONYM.match(alias) and sum(c.isupper() for c in alias) >= 2:
                    acronyms[alias] = {'acronym': alias, 'expansion': t['term'],
                                       'term': t['key']}
    for a in extra:
        if a.get('term') and a['term'] not in seen:
            errors.append(f'acronym {a["acronym"]}: term {a["term"]!r} does not exist')
        acronyms[a['acronym']] = a
    if errors:
        sys.exit('dictionary data errors:\n  ' + '\n  '.join(errors))
    return notes, sorted(acronyms.values(), key=lambda a: a['acronym'].lower())
...
def write_latex_extras(notes, acronyms, by_key):
    lines = ['% Generated by tools/build.py from dictionary/notes.yaml.',
             '\\domain{Useful Notes}', '']
    for n in notes:
        star = '*' if n.get('quote') else ''
        at = f'[{to_latex(n["at"])}]' if n.get('at') else ''
        lines.append(f'\\begin{{usefulnote}}{{{n["key"]}}}{{{to_latex(n["subject"])}}}')
        lines.append(f'  \\notetext{star}{at}{{{n["source"]}}}{{{to_latex(n["text"])}}}')
        if n.get('related'):
            lines.append(f'  \\related{{{",".join(n["related"])}}}')
        lines += ['\\end{usefulnote}', '']
    (ROOT / 'entries/notes.tex').write_text('\n'.join(lines))

    lines = ['% Generated by tools/build.py from dictionary/acronyms.yaml and aliases.',
             '\\domain{Acronyms}', '', '\\begin{acronymlist}']
    for a in acronyms:
        key = a.get('term')
        if not key:
            text = to_latex(a['expansion'])
        elif by_key[key]['term'] == a['expansion']:
            text = f'\\xref{{{key}}}, p.~\\pageref{{term:{key}}}'
        else:
            text = (f'{to_latex(a["expansion"])} \\quad {{\\small\\itshape see}} '
                    f'\\xref{{{key}}}, p.~\\pageref{{term:{key}}}')
        lines.append(f'  \\item[{a["acronym"]}] {text}')
    lines += ['\\end{acronymlist}', '']
    (ROOT / 'entries/acronyms.tex').write_text('\n'.join(lines))
...
def write_site(domains, sources, domain_of, notes, acronyms):
    ...
    lines = ['# Useful Notes', '',
             'Guidance and context that spans several terms.', '']
    for n in notes:
        src = sources[n['source']]
        cited_by[src.key].append(
            {'term': n['subject'], 'ref': f'{{ref}}`{n["subject"]} <note-{n["key"]}>`'})
        where = f', {n["at"]}' if n.get('at') else ''
        kind = ('[quoted]{.kind .kind-quoted}' if n.get('quote')
                else '[adapted]{.kind}')
        lines += [f'(note-{n["key"]})=', f'## {n["subject"]}', '']
        body = to_md(n['text']).split('\n\n')
        if n.get('quote'):
            lines += ['\n>\n'.join(f'> {para}' for para in body), '']
        else:
            lines += ['\n\n'.join(body), '']
        lines += ['{.note-source}',
        ...
        if n.get('related'):
            links = ' · '.join(term_ref(by_key[r]) for r in n['related'])
            lines += ['{.see-also}', f'See also: {links}', '']
    (out / 'notes.md').write_text('\n'.join(lines))

    lines = ['# Acronyms', '',
             'Acronyms used in this dictionary, each linked to its entry.', '',
             '| Acronym | Expanded | Entry |', '|---|---|---|']
    for a in acronyms:
        entry = term_ref(by_key[a['term']]) if a.get('term') else ''
        lines.append(f'| **{a["acronym"]}** | {a["expansion"]} | {entry} |')
    (out / 'acronyms.md').write_text('\n'.join(lines) + '\n')
    ...
    for dom in domains:
        ...
        for t in dom['terms']:
            ...
            for i, d in enumerate(t['definitions'], 1):
                ...
                cited_by[src.key].append({'term': t['term'], 'ref': term_ref(t)})
                text = to_md(d['text'])
                if d.get('quote'):
                    text = f'“{text}”'
    ...
    for title, group in groups:
        ...
        for s in sorted(group, key=lambda s: s.label.lower()):
            if not cited_by[s.key]:
                continue
            terms = sorted({c['ref']: c for c in cited_by[s.key]}.values(),
                           key=lambda c: c['term'].lower())
            lines += [f'(src-{s.key})=',
                      f'**{s.label}** — {s.reference()}',
                      '',
                      '{.cited-by}',
                      'Cited in: ' + ' · '.join(c['ref'] for c in terms),
                      '']
...
def main():
    ...
    notes, acronyms = load_extras(sources, domains, domain_of)
    by_key = {t['key']: t for dom in domains for t in dom['terms']}
    if target in ('latex', 'all'):
        write_latex(domains)
        write_latex_extras(notes, acronyms, by_key)
    if target in ('site', 'all'):
        write_site(domains, sources, domain_of, notes, acronyms)
